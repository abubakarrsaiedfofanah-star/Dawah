...
const pages = [
  { path: '/index.html', title: /Dawah|Dawa/i, marker: '#landingPage' },
  { path: '/officer.html', title: /Officer|Dawah|Dawa/i, marker: 'body' },
  { path: '/admin.html', title: /Admin|Dawah|Dawa/i, marker: '#adminLoginScreen' },
...
test.describe('visual smoke', () => {
  ...
  test('/index.html recovers from corrupted browser storage', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Dawah|Dawa/i);
  ...
  test('/index.html contact phone and email are clickable', async ({ page }) => {
    ...
    await expect(page.locator('#publicContactEmail')).toHaveAttribute('href', 'mailto:info@dawah.org');
    await expect(page.locator('#footerContactEmail')).toHaveAttribute('href', 'mailto:info@dawah.org');
  });
});
