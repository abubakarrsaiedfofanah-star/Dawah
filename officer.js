...
function blockOfficerStaticPreview() {
    showOfficerAlert(
        'Official access is blocked on this GitHub/static preview link. Use the real hosted Firebase/Firestore link for officer registration, approval, login, and password reset.',
        'warning'
    );
...
function registerOfficerLocally(data) {
    ...
    const member = {
        ...
        authEmail: window.DawaahCloud?.currentEmail?.() || data.email,
...
function handleOfficerResetPassword(event) {
    ...
    const resetRequest = frontendOnly
        ...
        : fetchOfficerApi('firestore-disabled-endpoint?action=resetPasswordWithCode', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function handleOfficerLogin(event) {
    ...
    if (frontendOnly) {
        try {
            ...
            const user = loginOfficerLocally(username, password, { authenticatedByFirebase: Boolean(window.DawaahCloud?.enabled) });
            localStorage.setItem('currentUser', JSON.stringify(user));
            localStorage.setItem('currentRole', user.role);
            localStorage.setItem('DawaahAccountClearVersion', '20260526-firebase-reset-v1');
