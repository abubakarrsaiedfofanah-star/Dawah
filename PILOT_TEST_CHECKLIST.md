...
- Backup button downloads Firestore JSON.
- Disabled legacy AI endpoints do not affect the current chat.

## Security

- Firebase Hosting serves only the static files in `firebase-public`.
- Firestore rules are deployed before inviting testers.
...
- Temporary `pilot.student.*@example.com` and `pilot.officer.*@example.com` accounts are deleted after live audits.
