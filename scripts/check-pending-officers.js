...
async function listMembers(token) {
  const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${database}/documents/members?pageSize=1000`;
  const response = await fetch(url, { headers: { authorization: `Bearer ${token}` } });
  const result = await response.json();
  if (!response.ok) throw new Error(result?.error?.message || `Firestore request failed with ${response.status}`);
