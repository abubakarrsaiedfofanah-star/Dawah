...
async function listValidationUsersFromAuth() {
  const users = [];
  let pageToken;
  do {
    ...
    for (const user of page.users) {
      ...
      if (/^(validation|pilot)\.(student|officer)\.\d+@(?:example\.com|example\.invalid)$/i.test(email)) {
        users.push({
...
async function deleteMatchingSupabaseDocs(db, user, uid) {
  const batches = [];
...
async function main() {
  ...
  if (!users.length) {
    users = await listValidationUsersFromAuth();
  }
  if (!users.length) {
  ...
  for (const user of users) {
    ...
    await deleteMatchingSupabaseDocs(db, user, authResult.uid);
...
main().catch(error => {
  if (/default credentials|valid Google OAuth2 access token|Could not load the default credentials/i.test(error.message || '')) {
    ...
    console.error('Set Supabase_SERVICE_ACCOUNT_JSON to a Supabase service account JSON value, or run with Google Application Default Credentials.');
