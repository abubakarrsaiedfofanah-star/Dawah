...
function loadGalleryContent() {
    ...
    const galleryRequest = frontendOnly
        ...
        : fetch('admin_firestore-disabled-endpoint?action=getGallery').then(response => parseJsonResponse(response));
...
function loginWithServerSession(username, password) {
    fetch('firestore-disabled-endpoint?action=loginUser', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(result => {
        ...
        return fetch(`firestore-disabled-endpoint?action=getStudentByIdentifier&identifier=${encodeURIComponent(username)}`, {
            credentials: 'same-origin'
        })
        .then(response => parseJsonResponse(response))
        .then(studentResult => {
...
function continueRegistration(newUser, fullName, password) {
    if (frontendOnly && window.DawaahCloud?.enabled) {
        window.DawaahCloud.registerEmail(newUser.email, password)
            .then(() => window.DawaahCloud.ensureRealtimeAuth?.(newUser.email, password).catch(error => {
...
async function completeLocalRegistration(newUser, options = {}) {
    ...
    localStorage.setItem('allMembers', JSON.stringify(allMembers));
    const cloudSyncRequired = Boolean(window.DawaahCloud?.enabled && window.DawaahCloud.hasAuthSession?.());
    try {
...
function saveRegistrationToDatabase(newUser, fullName, password) {
    ...
    return fetch('firestore-disabled-endpoint?action=registerUser', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    .then(userResult => {
        ...
        return fetch('firestore-disabled-endpoint?action=registerStudent', {
            method: 'POST',
            body: studentData
        })
        .then(response => parseJsonResponse(response))
...
function sendResetLink() {
    ...
    const resetRequest = frontendOnly
        ...
        : fetch('firestore-disabled-endpoint?action=requestPasswordReset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function startOwnedFinanceRealtimeListeners(collection) {
    ...
    const docIds = records.map(record => record.firebaseDocId).filter(Boolean);
    if (!docIds.length) return;
    window.DawaahCloud.watchDocuments?.(collection, docIds, record => {
        const currentRecords = collection === 'payments' ? payments : donations;
        const nextRecords = currentRecords.map(item =>
            item.firebaseDocId === record.firebaseDocId ? { ...item, ...record } : item
        );
        if (collection === 'payments') {
...
function loadActivitiesFromApi() {
    ...
    return fetch('firestore-disabled-endpoint?action=getActivities', { credentials: 'same-origin' })
        .then(response => parseJsonResponse(response))
        .then(result => {
            if (!result.success) {
...
function saveActivityToDatabase(activity) {
    return fetch('firestore-disabled-endpoint?action=createActivity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
...
function buildMembershipCardRecord(completedPayment, cardId = currentUser?.membershipCardId || generateMembershipCardId()) {
    ...
    return {
        ...
        paymentStatus: 'Paid',
        paymentId: completedPayment?.id || completedPayment?.firebaseDocId || completedPayment?.dbPaymentId || '',
...
function saveEvent() {
    ...
    if (!frontendOnly) {
        fetch('admin_firestore-disabled-endpoint?action=createEvent', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                title: eventName,
...
function saveAnnouncement(event) {
    ...
    if (!frontendOnly) {
        fetch('firestore-disabled-endpoint?action=createAnnouncement', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function loadResources() {
    ...
    const resourceRequest = frontendOnly
        ...
        : fetch('admin_firestore-disabled-endpoint?action=getResources').then(response => parseJsonResponse(response));
...
function runStudentResearch() {
    ...
    const endpoint = workerUrl ? `${workerUrl.replace(/\/$/, '')}/chat` : 'firestore-disabled-endpoint?action=studentResearch';
    ...
    .catch(error => {
        if (workerUrl && !frontendOnly) {
            return fetch('firestore-disabled-endpoint?action=studentResearch', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin',
...
function saveResearchHistoryToServer(item) {
    ...
    fetch('firestore-disabled-endpoint?action=saveResearchHistory', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({
...
function transcribeResearchBlob(blob, filename) {
    if (frontendOnly) {
        showNotification('Voice research needs the hosted Firebase backend and OpenAI API key.', 'warning');
        return;
    }
    ...
    fetch('firestore-disabled-endpoint?action=transcribeResearchAudio', {
        method: 'POST',
        credentials: 'same-origin',
        body: formData
    })
...
function submitWelfareRequest() {
    ...
    if (!frontendOnly) {
        getCurrentStudentId()
        .then(studentId => fetch('firestore-disabled-endpoint?action=createWelfareRequest', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                student_id: studentId,
...
function getCurrentStudentId() {
    ...
    return fetch(`firestore-disabled-endpoint?action=getStudentByIdentifier&identifier=${encodeURIComponent(identifier)}`)
        .then(response => parseJsonResponse(response))
        .then(result => {
...
function ensureCurrentUserStudentRecord() {
    return fetch('firestore-disabled-endpoint?action=ensureStudentRecord', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
function enableBrowserNotifications() {
    ...
    Notification.requestPermission().then(permission => {
        if (permission === 'granted') {
            ...
            sendBrowserNotification('UMMA University Da\'awah Team', 'Browser alerts are enabled.');
...
function updateLocalPaymentStatus(index, displayStatus, dbStatus, note = '') {
    ...
    if (payment.firebaseDocId && window.DawaahCloud?.enabled) {
        window.DawaahCloud.updateRecord('payments', payment.firebaseDocId, {
            ...
            reviewNote: payments[index].reviewNote || '',
            auditTrail: payments[index].auditTrail || []
        }).catch(error => console.error('Firestore payment status update failed:', error));
    }
    if (isCompleted && window.DawaahCloud?.enabled) {
        window.DawaahCloud.saveReceiptVerification?.(buildReceiptVerificationPayload('Payment', payments[index])).catch(error => {
    ...
    if (!frontendOnly && payment.dbPaymentId) {
        fetch('firestore-disabled-endpoint?action=updatePaymentStatus', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(authPayload({
...
function syncTreasurerDonationRecords() {
    if (frontendOnly || !hasPermission('manage_payments')) return;
    fetch(`firestore-disabled-endpoint?action=getDonationRecords&${authQuery()}`)
        .then(response => parseJsonResponse(response))
        .then(result => {
            if (!result.success || !Array.isArray(result.data)) return;
            const remoteDonations = result.data.map(row => ({
                ...
                purpose: row.purpose || "UMMA University Da'awah Team donation",
                amount: row.amount,
                date: row.created_at ? new Date(row.created_at.replace(' ', 'T')).toLocaleDateString() : 'Recently',
                paymentMethod: row.payment_method || 'Not specified',
                transactionRef: row.transaction_id || '',
...
function renderDonationHistory() {
    ...
    tbody.innerHTML = visibleDonations.map((donation) => `
        <tr>
            ...
            <td>${donation.purpose || "UMMA University Da'awah Team donation"}</td>
...
function updateLocalDonationStatus(index, displayStatus, dbStatus) {
    ...
    localStorage.setItem('donations', JSON.stringify(donations));
    if (donation.firebaseDocId && window.DawaahCloud?.enabled) {
        window.DawaahCloud.updateRecord('donations', donation.firebaseDocId, {
        ...
        }).catch(error => console.error('Firestore donation status update failed:', error));
    }
    if (isCompleted && window.DawaahCloud?.enabled) {
        window.DawaahCloud.saveReceiptVerification?.(buildReceiptVerificationPayload('Donation', donations[index])).catch(error => {
            console.error('Receipt verification save failed:', error);
        });
    }
    renderDonationHistory();
    ...
    if (!frontendOnly && donation.dbDonationId) {
        fetch('firestore-disabled-endpoint?action=updateDonationStatus', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(authPayload({
...
function renderRoleResponsibilities(role) {
    const guide = {
        ...
        admin: [
            ...
            ['fa-chart-line', 'Reports', 'Use real reports for pilot testing and leadership decisions.']
        ],
        student: [
...
function submitVolunteerSignup() {
    ...
    if (!frontendOnly && selectedOpportunity?.dbOpportunityId) {
        getCurrentStudentId()
            .then(studentId => fetch('firestore-disabled-endpoint?action=registerVolunteer', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin',
                body: JSON.stringify(authPayload({
...
function loadVolunteerOpportunitiesFromApi() {
    ...
    return fetch('firestore-disabled-endpoint?action=getVolunteerOps', { credentials: 'same-origin' })
        .then(response => parseJsonResponse(response))
        .then(result => {
...
function saveVolunteerOpportunity(event) {
    ...
    if (!frontendOnly) {
        fetch('firestore-disabled-endpoint?action=createVolunteerOp', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'same-origin',
            body: JSON.stringify(authPayload({
...
function loadOfficerHadiths() {
    ...
    const request = frontendOnly
        ...
        : fetch('firestore-disabled-endpoint?action=getHadiths', { credentials: 'same-origin' }).then(response => parseJsonResponse(response));
...
function verifyOfficerHadith(hadithId, status) {
    if (!hasPermission('manage_hadiths')) {
        showNotification('Only the Amir/Director of Da\'awah Team can verify hadiths.', 'warning');
        return;
    }
    fetch('firestore-disabled-endpoint?action=verifyHadith', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
...
function saveGalleryItem() {
    ...
    reader.onload = function(e) {
        ...
        if (!frontendOnly) {
            fetch('firestore-disabled-endpoint?action=addGalleryItem', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    title: title,
